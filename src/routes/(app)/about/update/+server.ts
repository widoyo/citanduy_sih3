...
export const GET = async ({request, platform}) => {
    ...
    const prakiraanHtml = await fetchPrakiraan('https://www.bmkg.go.id/cuaca/prakiraan-cuaca/32');
    const prakiraanList = await parsePrakiraan(prakiraanHtml);

    const prakiraanHtml2 = await fetchPrakiraan('https://www.bmkg.go.id/cuaca/prakiraan-cuaca/33');
